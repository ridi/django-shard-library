...
class LockServiceTestCase(TestCase):
    ...
    def test_get_lock_manager(self):
        ...
        self.assertEqual(config.SHARD_TRANSMIT_LOCK_MANAGER_CLASS, FakeLockManager.__module__ + '.' + FakeLockManager.__name__)

    def test_make_key(self):
        self.assertEqual(self.lock_manager.key, lock_service._make_lock_key(self.model_name, self.database_alias))
        self.assertEqual(self.lock_manager.ttl, config.SHARD_TRANSMIT_LOCK_TTL)
    ...
    def test_import_lock_manager_class(self):
        ...
        self.assertEqual(config.SHARD_TRANSMIT_LOCK_MANAGER_CLASS, lock_manager_class.__module__ + '.' + lock_manager_class.__name__)

    @patch('shard_static.config.SHARD_TRANSMIT_LOCK_MANAGER_CLASS', 'not.exists.package')
    def test_raise_import_error_in_import_lock_manager_class(self):
        with self.assertRaises(ImportError):
            lock_service._import_lock_manager_class()

    @patch('shard_static.config.SHARD_TRANSMIT_LOCK_MANAGER_CLASS', 'tests.lock.NotExistLockManager')
    def test_raise_attribute_error_in_import_lock_manager_class(self):
        with self.assertRaises(AttributeError):
            lock_service._import_lock_manager_class()

    @patch('shard_static.config.SHARD_TRANSMIT_LOCK_MANAGER_CLASS', None)
